"""Common config and helpers for gemma-anthology EP01 production scripts."""
...
SHOTLIST = PROJECT / "shotlists" / "episode-01.md"
...
DELIVERABLES = PROJECT / "DELIVERABLES" / "EP01"
...
NEGATIVE = (
    ...
    "cartoon, anime soft, signature, bad anatomy, duplicate figures"
)

STYLE_TAG = (
    "Cyberpunk-baroque cinematic animation frame, 35mm anamorphic lens with "
    "slight grain, high contrast lighting with deep shadows and saturated "
    "neon highlights, stylized graphic-novel rendering with cinematic "
    "lighting weight."
)
...
def poll_job(job_id: str, timeout: int = 600) -> bytes:
    ...
    while time.time() - start < timeout:
        ...
        if st == "completed":
            r = requests.get(f"{COMFYUI_BASE}/jobs/{job_id}/result", timeout=60)
            return r.content
