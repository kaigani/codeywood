#!/usr/bin/env python3
"""
Generate 12 EP01 video clips via LTX-2.3 with first_frame + audio + prompt.
...
def build_motion_prompt(shot: dict) -> str:
    ...
    if shot.get("dialogue") and shot.get("speaker"):
        speaker_label = shot["speaker"].title()
...
def poll_jobs(jobs: dict, timeout: int = 1200) -> list[str]:
    pending = dict(jobs)
    failed = []
    while pending:
        still: dict = {}
...
def main():
    ...
    print("=" * 70)
    print(f"GENERATE CLIPS — {WORKFLOW} — {len(shots)} shots — total {total_dur}s")
    print("=" * 70)
    ...
    for shot in shots:
        ...
        if not audio.exists():
            print(f"  Shot {sid:2d}: NO AUDIO MIX {audio.name}")
            failed.append(name)
            continue

        adur = probe_duration_ff(audio)
        dur = max(int(round(adur)), shot["duration"])
        dur = min(dur, 20)  # LTX-2.3 hard cap
